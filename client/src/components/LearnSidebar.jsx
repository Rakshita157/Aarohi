...
import { modules as modulesEn } from '../pages/data/lessons';
import { modules as modulesHi } from '../pages/data/lessons_hi';
...
const getMotivation = (pct, t) => {
  if (pct === 0) return t('sidebar.motivation0');
  if (pct <= 25) return t('sidebar.motivation25');
  if (pct <= 50) return t('sidebar.motivation50');
  if (pct <= 75) return t('sidebar.motivation75');
  return t('sidebar.motivation100');
};
...
const LearnSidebar = () => {
  ...
  const sidebarFeatures = [
    { Icon: ShieldIcon, title: t('sidebar.features.f1Title'), desc: t('sidebar.features.f1Desc') },
    { Icon: BookIcon, title: t('sidebar.features.f2Title'), desc: t('sidebar.features.f2Desc') },
    { Icon: ClockIcon, title: t('sidebar.features.f3Title'), desc: t('sidebar.features.f3Desc') },
    { Icon: BotIcon, title: t('sidebar.features.f4Title'), desc: t('sidebar.features.f4Desc') },
  ];
  ...
  return (
    <aside className="learn-sidebar">
      ...
      <div className="sidebar-card">
        <h3 className="sidebar-card-title">{t('sidebar.progressTitle')}</h3>
        ...
        <div className="sidebar-cta-wrap">
          ...
          >
            {allDone ? t('sidebar.reviewModules') : t('sidebar.continueLearning')}
          </Link>
        </div>
      </div>
      ...
      {/* Card 2 — Why Learn */}
      <div className="sidebar-card">
        <h3 className="sidebar-card-title">{t('sidebar.whyLearnTitle')}</h3>
        <p className="sidebar-card-sub">{t('sidebar.whyLearnSub')}</p>
      ...
      <div className="sidebar-card">
        <h3 className="sidebar-card-title">{t('sidebar.learningPathTitle')}</h3>
        <div className="sidebar-path">
          <div className="path-line" />
          {visiblePathSteps.map((step, i) => {
            const isCompleted = completed.has(step.id);
            const isCurrent = step.id === currentModuleId;
            const label = modules.find((m) => m.id === step.id)?.title || (step.id === 8 ? t('nav.askSakhi') : step.label);

            return (
              <Link
                key={step.id}
                to={step.id === 8 ? '/ask-sakhi' : `/learn/${step.id}`}
              ...
              >
                ...
                <span className="path-step-label">{label}</span>
          ...
          {pathSteps.length > 5 && (
            <button className="path-toggle-btn" onClick={() => setShowAllPath(p => !p)}>
              {showAllPath ? t('sidebar.showLess') : t('sidebar.showAllSteps')}
